...
We propose that given a secret, *T*&minus;2 shares be generated randomly and the remaining shares be computed in such a way that *f*(255) encodes the shared secret and *f*(254) encodes the digest of the shared secret. Encoding the digest makes it possible to verify that the shared secret has been correctly recovered.
...
5. Let *R* be the last *n* &minus; 4 bytes of *D*. If the first 4 bytes of HMAC-SHA256(key=*R*, msg=*S*) are equal to the first four bytes of *D*, then return *S*, otherwise abort.
