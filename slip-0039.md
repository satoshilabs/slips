...
|-------------------|-------------------------|-------------|---------------------------|----------------|
| 20 bits           | 6 bits                  | 6 bits      | 8*n* bits   | 30 bits        |
...
* threshold (*T*) field indicates how many shares are needed to reconstruct the secret the actual value is encoded as *T*&minus;1, so a value of 0 indicates that a single share is needed (*T*=1), a value of 1 indicates that two shares are needed (*T*=2) etc.
...
The round function `F(R_i)` is defined as follows<sup>[4](#KDFParam)</sup>:
```
