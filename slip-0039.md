...
Since Shamir's secret sharing scheme is applied separately to each of the *n* bytes of the pre-master secret, we work with *y*<sub>*i*</sub> as a vector of *n* values, where *y*<sub>*i*</sub>[*k*] = *f<sub>k</sub>*(*x<sub>i</sub>*), 1 &le; *k* &le; *n*, and *f<sub>k</sub>* is the polynomial in the *k*-th instance of the scheme.
...
2. Compute the pre-master secret *PMS* = MSDF<sup>-1</sup>(*S*, *P*, *id*).
3. If *T* is 1, then let *s*<sub>1</sub> = *PMS* and return.
4. Let the *pre-master digest* (*PMD*) be the first four bytes of SHA-256(SHA-256(*PMS*)) followed by *n* &minus; 4 randomly generated bytes.
5. Let *s*<sub>1</sub>, ... , *s*<sub>*T*&minus;2</sub> &in; GF(256)<sup>*n*</sup> be generated randomly.
6. For *i* such that *T* &minus; 2 &le; *i* &le; *M* compute *s<sub>i</sub>* = Interpolation(*i* &minus; 1, {(0, *s*<sub>1</sub>), ... , (*T* &minus; 3,*s*<sub>*T*&minus;2</sub>), (254, *PMD*), (255, *PMS*)}).
...
Given *m* shares, the following conditions are checked before proceeding further:
...
1. Let *shares* = {(*I*<sub>1</sub>, *s*<sub>1</sub>), ... , (*I<sub>m</sub>*, *s<sub>m</sub>*)}, where *I<sub>i</sub>* are the share indices and *s<sub>i</sub>* are the share values.
2. Compute *PMS* = Interpolation(255, *shares*).
3. Compute *PMD* = Interpolation(254, *shares*).
4. If the first four bytes of SHA-256(SHA-256(*PMS*)) are equal to the first four bytes of *PMD*, then return *PMS*, otherwise abort.
