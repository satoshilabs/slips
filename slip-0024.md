...
Until the required verification has completed successfully, such information is a claim, not a fact.
...
A statement memo asserts a property of the payment request, of the transaction used to satisfy the payment request, or of an object relevant to that transaction, which is verified by the payer or their wallet.
Until the specified verification has completed successfully, the contents of a statement memo are a claim, not a fact.

The following statement types are defined.

##### SIMULATION

A `SIMULATION` statement describes the expected effects of executing the transaction in a specified blockchain state.
A simulation result may include assets transferred or received, balance changes, approvals, contract calls, state changes, and other execution effects relevant to authorization.
A simulation establishes an expected result relative to the state and execution context against which it was performed. It does not by itself guarantee that execution will produce the same result if relevant state changes before execution.

##### ATTESTATION

An `ATTESTATION` statement asserts that a property of the payment request, transaction, recipient, asset, contract, blockchain state, or another object relevant to the payment is true.
Examples include identity or metadata of an asset or contract, a property of a recipient or destination, a property of blockchain state, a policy or compliance property, a security property, or an assertion made by an external party.
An attestation concerning mutable state MUST identify the state to which the assertion applies if that distinction is relevant to authorization.
...
A trusted party is a party authorized by the payer's wallet to sign payment requests.
...
A valid payment request signature establishes only that the trusted party signed the payment request and therefore committed to its contents, including any statement memos it contains. It does not by itself establish the truth of a statement memo. The trust assumptions, if any, required to establish a statement are defined by its verification method.
...
1. The wallet verifies that it supports the statement type and verification method.
2. The wallet verifies that the statement subject applies to the payment request, transaction, or other object identified by the statement.
3. The wallet obtains any verification data or other evidence required by the verification method and verifies it according to that method.
4. If a statement hash is present, the wallet computes the canonical verification result and verifies that its SHA-256 hash matches the statement hash.
5. Only after successful verification may the wallet rely on the statement when presenting or authorizing the payment.

If verification of a statement on which authorization depends fails, the wallet MUST NOT authorize the transaction as satisfying the payment request.
A statement memo is not inherently required if the wallet can independently establish the information needed to authorize the payment.
...
* *statementType* (4 bytes): 32-bit identifier of the statement type in little-endian byte order.
* *subject* (length-prefixed binary string): canonical encoding of the statement subject as defined for the applicable combination of statement type and verification method.
* *statementHash* (length-prefixed binary string): either a zero-length string if no statement hash is provided, or a 32-byte SHA-256 hash of the canonical verification result.
...
The following statement type identifiers are defined:

* `1` - `SIMULATION`
* `2` - `ATTESTATION`

Statement type identifier `0` is reserved. Additional statement types MAY be defined by future revisions of this specification.

Verification methods are identified by a 32-bit unsigned integer. Their definitions and registry are outside the scope of this specification.

Verification methods may establish statements through mechanisms such as signed attestations, trusted execution environments, or cryptographic proofs of computation.

A specification defining a verification method MUST define the statement types to which the method applies, the meaning and canonical encoding of the subject and verification result, whether a statement hash is required, the meaning and encoding of verification data, the verification procedure, how evidence is bound to the subject and result, applicable trust assumptions and domain separation, any freshness and replay requirements, and conditions under which verification fails.
A verification method MAY permit evidence to be supplied separately from the payment request, provided that the method defines how that evidence is authenticated and bound to the statement.

A wallet MUST reject a statement memo whose statement hash has a length other than zero or 32 bytes.
A wallet MUST reject a statement memo if the specified verification method does not support the specified statement type.
