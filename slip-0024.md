...
A payment request authenticates its requested outputs, but not security-relevant effects which are not fully determined by those outputs.
...
A payment request does not establish the absence of semantics which belong to a protocol that neither the wallet nor an applicable verification method implements. A wallet MUST NOT authorize a transaction as satisfying a payment request if this specification defines no applicable requested-output encoding for its transaction, authorization or payment-instruction form, or if the transaction contains a transaction-level component whose semantics the wallet does not implement. A wallet MAY refuse to authorize a transaction whose security semantics it cannot establish as its policy requires.
...
Every stated asset bound MUST be presented and confirmed, and any deferred authority MUST be displayed. If a bound is unbounded, the wallet MUST either refuse the output or require the payer to confirm that the amount of that asset at risk is unbounded. The wallet MUST resolve *attester* to a party which it trusts for the verification method, and when presenting the result it MUST present its own identity for that party, rather than identity information supplied by the payment request.
...
An EVM output MUST be treated as not bounded by its amount unless the wallet establishes that, beyond the transaction value and native transaction fee, the transaction leaves no gas for execution at the destination and causes no transaction-level processing which may affect the payer's assets or confer authority over them, such as an authorization list, a protocol or system-contract request which affects consensus-layer state, or a fee mechanism which executes code. The wallet MUST establish this from the transaction being signed and network rules it implements; information supplied by the payment request does not establish it, and the current absence of code at the destination is not sufficient, because code or delegated code may exist when the transaction executes.
