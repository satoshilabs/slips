...
A wallet MUST NOT suppress, replace or relabel a confirmation which it would otherwise require for a security-relevant property of the transaction or signing operation, unless the effect of that property is fully determined by a requested output authenticated by the payment request. Otherwise, the wallet MUST present the property to the payer as it would if no payment request were provided, or MUST refuse to authorize the transaction as satisfying the payment request.
