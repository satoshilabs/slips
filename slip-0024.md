...
A payment request does not establish the absence of semantics which belong to a protocol that the wallet does not implement. A wallet MAY refuse to authorize a transaction whose security semantics it cannot establish as its policy requires.
...
* *verificationMethod* (4 bytes): 32-bit identifier of the verification method in little-endian byte order.
* *attester* (length-prefixed binary string): identifier of the party which attested to the verification result.
