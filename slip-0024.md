...
A statement memo commits to the result of applying a verification method to selected requested outputs or, if none are selected, to a state assertion. Several statement memos MAY apply to the same requested output. The wallet MUST verify a statement memo before relying on it, see [Payload hash and statement hash](#payload-hash-and-statement-hash).
...
where *verificationMethod* is encoded as a 32-bit little-endian integer, *attester* is length-prefixed and *verificationResult* is the canonical result encoding. The wallet MUST derive the payload, verification context and verification result itself, and MUST NOT rely on a statement memo unless its *statementHash* equals the value computed above.
...
A raw output specification is a coin-specific encoding of the destination and transaction data needed to specify an output when an address alone is insufficient. A wallet MUST include it in the computation of *outputsHash* only if it implements the encoding and it matches the corresponding output of the transaction being signed. The wallet MUST allow the payer to inspect such an output in the same way as it would present the output if no payment request were provided.
