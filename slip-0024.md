...
An output is normally specified by its destination address. Some outputs, such as Bitcoin OP_RETURN outputs, have no address form. Such outputs are specified by a *raw output specification*, whose format is defined for each coin type, see [Raw output specifications](#raw-output-specifications). If the destination of an output has an address form, then the address form MUST be used.
