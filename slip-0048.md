...
The Role comes prior to the Account index so that a role-specific parent key can be derived which allows to derive child keys that do not interfere with other roles.
...
Thus, software should prevent an update of an account with a specific key (see below) if a previous key does not have an account associated with it.
...
Depending on the blockchain's API and available resources, account discovery could be done with multiple accounts (respect the gap limit) at once. With a gap limit of 5 for account's and a gap limit of 5 for keys, we would need to scan for 25 keys. Combined with bloom filtering, the amount of data could be reduced at the expense of a single step lookup.
